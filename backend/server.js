  ...
  return new Promise((resolve, reject) => {
    https.get(FFMPEG_URL, (response) => {
      ...
      if (response.statusCode !== 200) {
        reject(`[Backend] Error downloading FFmpeg: ${response.statusCode}`);
        return;
      }
      ...
      file.on('finish', async () => {
        ...
        if (isWindows) {
          ...
          if (!ffmpegFolder) return reject("[Backend] FFmpeg folder not found.");

          const extractedPath = path.join(TEMP_DIR, ffmpegFolder, "bin", "ffmpeg.exe");
          if (!fs.existsSync(extractedPath)) return reject("[Backend] ffmpeg.exe not found.");
          ...
          exec(`tar -xf ${ZIP_PATH} -C ${TEMP_DIR}`, (err) => {
            if (err) return reject("[Backend] Error extracting FFmpeg on Linux.");
            ...
            if (!fs.existsSync(extractedPath)) return reject("[Backend] FFmpeg binary not found.");
      ...
      file.on('error', (err) => reject(`[Backend] Error writing file: ${err}`));
    }).on('error', (err) => reject(`[Backend] Download request error: ${err}`));
  });
};
...
app.get('/get_spotify_token', async (req, res) => {
  try {
    ...
    if (!data.accessToken) throw new Error("[Backend] Failed to retrieve Spotify token.");

    console.log("[Backend] Spotify token obtained successfully.");
    res.json({ accessToken: data.accessToken });

  } catch (error) {
    console.error("[Backend] Error getting Spotify token:", error);
    res.status(500).json({ error: "Could not retrieve Spotify token." });
  }
});
...
}).catch(error => console.error("[Backend] Error downloading FFmpeg:", error));
